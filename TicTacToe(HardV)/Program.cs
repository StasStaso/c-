    ...
    class Cross
    {
        ...
        private static bool CheckWin(char sym)
        {
            for (int i = 0; i < SIZE_X; i++)
            {
                ...
                {   
                    if (CheckLine(i, j, 1, 0, toWin, sym)) return true;   
                    if (CheckLine(i, j, 1, 1, toWin, sym)) return true;   
                    if (CheckLine(i, j, 0, 1, toWin, sym)) return true;   
                    if (CheckLine(i, j, 1, -1, toWin, sym)) return true;  
                }
            }
            return false;
        ...
        private static bool CheckLine(int x, int y, int direction_x, int direction_y, int depthOfCheck, char sym)
        {
            ...
            if (!IsCellInField(endX, endY)) return false; 
